...
> - ✅ **刘海屏/安全区全局适配**：已完成 ✅（2026-08-04，commit eb7b77e）：新增需求。**最终方案**：CSS env()/constant() 与 `--status-bar-height` 在开发者工具模拟器中不可靠（不模拟 safe-area、状态栏高度不准），改用 **`uni.getSystemInfoSync()` 动态取值**——首页搜索栏 `paddingTop = statusBarHeight`（模拟器 iPhone 12 = 47px）；tabbar 底部占位 `screenHeight - safeArea.bottom`（iPhone 12 = 34px），iOS ≥812 高且 API 无安全区时兜底 34px；全局 `src/style/index.scss` 保留 `.pt-safe/.pb-safe/.p-safe` 类（原生 CSS 多声明 fallback）供 H5 iOS Safari 等场景使用。⚠️ 坑：UnoCSS rules 的数组值会合并成逗号列表（`padding-top: a,b,c` 无效声明），无法做 CSS fallback；修改后需在模拟器切 iPhone 12 系列验证。
